...
export async function generateFeedback(request: FeedbackRequest): Promise<FeedbackResponse> {
    try {
        ...
        const response = await fetch('https://interview-ai.stepful.com/v1/chat/completions', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${process.env.STEPFUL_EMAIL || "90lineng@gmail.com"}`
            },
            body: JSON.stringify({
                model: "gpt-4o-2024-08-06",
                messages: [
                    { role: "system", content: "You are a helpful educational tutor. Provide constructive feedback for incorrect quiz answers. Be encouraging but honest about mistakes. Keep responses concise and educational." },
        ...
        const content = data.choices?.[0]?.message?.content || "Unable to generate feedback at this time.";
        return parseFeedbackResponse(content, request);
    } catch (error) {
        return {
